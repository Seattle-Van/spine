(function() {
  var Collection, Instance, ManyToMany, Singleton, singularize, underscore;
  ...
  Collection = (function() {
    ...
    Collection.prototype.find = function(id) {
      var records;
      var _this = this;
      records = this.model.select(function(rec) {
  ...
  Singleton = (function() {
    ...
    Singleton.prototype.update = function(value) {
      if (value != null) value[this.fkey] = this.id;
      return value;
    };
  ...
  ManyToMany = (function() {
    ...
    __extends(ManyToMany, Spine.Module);

    function ManyToMany(options) {
      var key, value, _base, _name, _ref;
      if (options == null) options = {};
      for (key in options) {
        value = options[key];
        this[key] = value;
      }
      if ((_ref = (_base = this.record)[_name = this.fkey]) == null) {
        _base[_name] = [];
      }
    }

    ManyToMany.prototype.add = function(mod) {
      this.record[this.fkey].push(mod.id);
      return this.record.update();
    };

    ManyToMany.prototype.remove = function(mod) {
      var r, t, _ref;
      r = this.record;
      if ((t = r[this.fkey].indexOf(mod.id)) > -1) {
        [].splice.apply(r[this.fkey], [t, t - t + 1].concat(_ref = [])), _ref;
      }
      return r.update();
    };

    ManyToMany.prototype.all = function() {
      var id, _i, _len, _ref, _results;
      _ref = this.record[this.fkey];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        id = _ref[_i];
        _results.push(this.model.find(id));
      }
      return _results;
    };

    ManyToMany.prototype.first = function() {
      return this.all()[0];
    };

    ManyToMany.prototype.last = function() {
      var values;
      values = this.all();
      return values[values.length - 1];
    };

    ManyToMany.prototype.find = function(id) {
      var records;
      var _this = this;
      records = this.model.select(function(rec) {
        return _this.associated(rec) && rec.id === id;
    ...
    ManyToMany.prototype.select = function(cb) {
      var _this = this;
      return this.model.select(function(rec) {
        return _this.associated(rec) && cb(rec);
      });
    ...
    ManyToMany.prototype.refresh = function(values) {
      var value;
      this.record[this.fkey] = (function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = values.length; _i < _len; _i++) {
          value = values[_i];
          _results.push(value.id);
        }
        return _results;
      })();
      return this.model.trigger('refresh');
    ...
    ManyToMany.prototype.create = function(record) {
      var mod;
      mod = this.model.create(record);
      this.add(mod);
      return mod;
    };

    ManyToMany.prototype.associated = function(record) {
      var _ref;
  ...
  Spine.Model.extend({
    ...
    manyToMany: function(name, model, fkey) {
      var association;
      if (fkey == null) fkey = "" + (singularize(name)) + "_ids";
      association = function(record) {
